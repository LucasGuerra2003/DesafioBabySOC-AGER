		...
		<div class="row">
			<table class="table table-light table-striped align-middle">
				...
				<tbody>
					<s:iterator value="funcionarios">
						<tr>
							<td>${rowid}</td>
							<td>${nome}</td>
							<td class="text-end"><s:url action="buscarExameFuncionario"
									var="exames">
									<s:param name="exameFuncionarioVo.funcionarioId" value="rowid"></s:param>
								</s:url> <a href="${exames}" class="btn btn-success text-white"> <s:text
										name="label.exames" />
							</a> <s:url action="editarFuncionarios" var="editar">
				...
				<tfoot class="table-secondary">
					<tr>
						<td colspan="2"><s:url action="todosExames" var="todos" /> <a
							href="${todos}" class="btn btn-success"> <s:text
									name="label.exames" />
						</a></td>
						<td colspan="2" class="text-end">
							<div class="d-flex justify-content-end">
								<s:url action="todoRelatoriosExame" var="relatorio" />
								<a href="${relatorio}" class="btn btn-warning me-2"><s:text name="label.relatorio.exames" /></a>

								<s:url action="novoFuncionarios" var="novo" />
								<a href="${novo}" class="btn btn-primary"> <s:text
										name="label.novofuncionario" />
								</a>
							</div>
						</td>
					</tr>
	...
	<script>
		document.addEventListener("DOMContentLoaded", function() {
			  ...
			  modal.addEventListener("show.bs.modal", function(event) {
			    var rowid = event.relatedTarget.dataset.rowid;
			    var url = "/avaliacao/excluirFuncionarios.action";
			    url += "?funcionarioVo.rowid=" + rowid;
			    var excluirLink = document.getElementById("excluir");
			    excluirLink.href = url;
			  });
			});
		</script>
</body>
